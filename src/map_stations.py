"""
map_stations.py — Phase 5: interactive map of public EV charging stations,
filterable by connector type (Plotly legend) and region.

Usage:
    python src/map_stations.py
    python src/map_stations.py --bundesland Bayern
    python src/map_stations.py --max-stations 5000
"""
...
# Drawn in this order so DC fast-charging points render on top of the far more
# numerous AC points rather than being buried underneath them.
...
def classify_connector(df: pd.DataFrame) -> pd.Series:
    """Categorise each station by the best (fastest) connector present anywhere
    on it — that determines whether it can rapid-charge an EV."""
...
def build_map(
    ...
    """Build a Scattermap figure. `category_labels` optionally overrides each
    CONNECTOR_CATEGORIES key with a display label (e.g. a translation)."""
...
def main() -> None:
    ...
    parser.add_argument("--bundesland", default=None,
                        help="Filter to one German state (e.g. Bayern, Berlin)")
    parser.add_argument("--max-stations", type=int, default=None,
                        help="Randomly sample down to this many stations")
    args = parser.parse_args()

    df = load_stations()
    if args.max_stations and len(df) > args.max_stations:
        df = df.sample(args.max_stations, random_state=0)
        logger.info("Sampled down to %d stations", len(df))
