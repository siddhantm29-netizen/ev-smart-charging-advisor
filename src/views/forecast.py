"""Forecast page: next-48h price and renewable-share charts plus the backtest
comparison table."""
...
def render(lang: str) -> None:
    ...
    st.plotly_chart(
        _line_chart(forecast_df, "price_forecast",
                    t("forecast_price_chart_title", lang),
                    t("forecast_price_axis", lang), PALETTE["xgboost"]),
        use_container_width=True,
    )
    st.plotly_chart(
        _line_chart(forecast_df, "renewable_share_forecast",
                    t("forecast_renewable_chart_title", lang),
