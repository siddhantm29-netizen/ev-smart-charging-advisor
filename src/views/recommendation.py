...
def _recommendation_banner(windows: list[dict], origin: pd.Timestamp, lang: str) -> str:
    ...
    if hours_away <= 1:
        return t("recommendation_now", lang,
                 end=fmt_datetime(best["end"], lang),
                 duration=best["duration_h"], price=price, renewable=renewable)
    return t("recommendation_wait", lang,
             hours=round(hours_away), start=fmt_datetime(best["start"], lang),
             duration=best["duration_h"], price=price, renewable=renewable)
...
def _chart_with_windows(scored_df: pd.DataFrame, windows: list[dict], lang: str) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=PALETTE["xgboost"], width=2),
    ))
    for w in windows:
