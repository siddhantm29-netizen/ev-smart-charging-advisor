...
app. Two languages: English ("en") and German ("de").
"""
...
TRANSLATIONS = {
    "en": {
        ...
        "windows_heading":         "Ranked windows",
                "metric_best_time":        "Best Time",
        "metric_avg_price":        "Avg. Price",
        "metric_green_energy":     "Green Energy",
        "metric_best_time":        "Beste Zeit",
        "metric_avg_price":        "Ø Preis",
        "metric_green_energy":     "Grüner Strom",
"window_col_rank":         "#",
...
def fmt_number(value: float, lang: str, decimals: int = 1) -> str:
    ...
    German: comma decimal + period thousands; English: the reverse."""
...
def fmt_datetime(dt: pd.Timestamp, lang: str, with_weekday: bool = True) -> str:
    """Format a timestamp with locale-correct weekday abbreviations.
    Abbreviations are substituted manually rather than via strftime's locale,
    which may not be installed on deployment hosts."""
    if lang == "de":
